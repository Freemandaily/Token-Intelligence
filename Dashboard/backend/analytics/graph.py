...
from data.transfers import get_transfers_for_token, get_token_metadata
...
def build_graph(transfers_df: pd.DataFrame) -> nx.DiGraph:
    ...
    edges = df.groupby(["from_address", "to_address"]).agg(
        transfer_count=("amount", "count"),
...
def compute_graph_analysis(token_address: str, db: Session, top_n: int = 250) -> dict:
    """Compute a graph limited to the top N holders.

    Returns nodes, edges and basic statistics for visualization.
    """
    transfers = get_transfers_for_token(token_address, db)
    balances = get_wallet_balances(token_address, db, transfers)
    ...
    touching_top_holders = transfers[
        transfers["from_address"].isin(top_holders_set) |
        transfers["to_address"].isin(top_holders_set)
    ]
    ...
    filtered = transfers[
        transfers["from_address"].isin(allowed_nodes) &
        transfers["to_address"].isin(allowed_nodes)
    ]
    ...
    transfers["amount_num"] = pd.to_numeric(transfers["amount"], errors="coerce").fillna(0.0)
    
    in_stats = transfers.groupby("to_address").agg(
        total_volume_in=("amount_num", "sum"),
        unique_wallets_in=("from_address", "nunique")
    ).to_dict("index")
    
    out_stats = transfers.groupby("from_address").agg(
    ...
    for _, row in transfers.iterrows():
        if pd.notna(row.get("from_address_name")):
            address_names[row["from_address"]] = row["from_address_name"]
        if pd.notna(row.get("to_address_name")):
            address_names[row["to_address"]] = row["to_address_name"]
    ...
    meta = get_token_metadata(token_address, db)
...
def compute_top_holders_graph(token_address: str, db: Session, top_n: int = 250) -> dict:
    """Alias for compute_graph_analysis keeping older endpoint name."""
    return compute_graph_analysis(token_address, db, top_n)
